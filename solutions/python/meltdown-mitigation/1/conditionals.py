...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    criticality_measure = temperature * neutrons_produced_per_second

    if criticality_measure < 0.90 * threshold:
        return 'LOW'
    elif (criticality_measure >= threshold - 0.10*threshold)  and (criticality_measure <= threshold + 0.10*threshold) or (criticality_measure == threshold):
        return 'NORMAL'
    else:
        return 'DANGER'
